...
1. What is the purpose of using _sessions_?
    ...
    Sessions allow the server to store information about the client, so the authentication data is persistant and the client doesn't have to re-enter it on every server request.
...
1. What does bcrypt do to slow down attackers?
    ...
    Bcrypt hashes our passwords multiple times, this makes guessing passwords much harder for attackers. Since hashing only works one way, someone trying to guess the password would also have to calculate multiple hashes.

1. What are the three parts of the JSON Web Token?

    The three parts of JWT are the header, the payload and the signature.
